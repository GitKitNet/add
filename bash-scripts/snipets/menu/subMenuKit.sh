...
${BLUE}_______________________${NC}${GREEN}________________${NC}
${BLUE}  ______           __ ${NC}${GREEN}__    _  __      ${NC}
${BLUE} /_  __/__  ___   / /${NC}${GREEN}/ /__ (_)  /_     ${NC}
${BLUE}  / / / _ \/ _ \ / /${NC}${GREEN}/  '_// /  __/     ${NC}
${BLUE} /_/  \___/\___//_/${NC}${GREEN}/_/\_\/_/\__/       ${NC}
${BLUE}__________________${NC}${GREEN}_____________________${NC}
"; }
function pause() { read -p "Press [Enter] key to continue..." fackEnterKey; }
function wait() { echo -e "Press [ANY] key to continue..? \n" && read -s -n 1; }
function TIMER() { if [[ "$1" =~ ^[[:digit:]]+$ ]]; then T=${1}; else T=5; fi; S="$((1 * ${T}))" && C='\033[0K\r'; while [ $S -gt 0 ]; do echo -ne "${GREEN} Please wait: ${S}${C} ${NC}"; sleep 1 && : $((S--)); done; }
function OSrelease {
...
## ========= Sub Menu 1 ========= ##
function MENUsub_ONE() {
	echo -e "\n\t${BLUE}STAGE MENU 1: ${NC}\n"
	echo -e "
	\t1. Start installation LAMP (Apache, MySQL, PHP)
	\t2. Start installation LAMP & Wordpress
	\t2. MENU Stage 1 - Part 3
	\n\t0. Back"
	echo -n -e "\n\tSelection: "
}
## ========= Sub Menu 2 ========= ##
function MENUsub_TWO() {
	echo -e "\n\t${GREEN}STAGE MENU 2: ${NC}\n"
	echo -e "
	\t1. MENU Stage 2 - Part 1
	\t2. MENU Stage 2 - Part 2
	\t2. MENU Stage 2 - Part 3
	\n\t0. Back"
	echo -n -e "\n\tSelection: "
}
## ========= Sub Menu 3 ========= ##
function MENUsub_THREE() {
	echo -e "\n\t${YELOW}STAGE MENU 3: ${NC}\n"
	echo -e "
	\t1. MENU Stage 3 - Part 1
	\t2. MENU Stage 3 - Part 2
	\t2. MENU Stage 3 - Part 3
	\n\t0. Back"
	echo -n -e "\n\tSelection: "
}
...
do
	...
	case $opt in
		...
		do
			...
			read -n1 opt;
			case $opt in
				1) echo "COMMANDS Stage 1 - Part 1" && wait;;
				2) echo "COMMANDS Stage 1 - Part 2" && wait;;
				3) echo "COMMANDS Stage 1 - Part 2" && wait;;
				/q | q | 0)break;;
				*) ;;
			esac
		done
      ...
      2) echo -e "# submenu: Convert"
		...
		do
			...
			read -n1 opt;
			case $opt in
				1) echo "COMMANDS Stage 2 - Part 1" && wait;;
				2) echo "COMMANDS Stage 2 - Part 2" && wait;;
				3) echo "COMMANDS Stage 2 - Part 2" && wait;;
				/q | q | 0)break;;
				*) ;;
			esac
		done
      ...
      3) echo -e "# submenu: Verify"
		...
		do
			...
			read -n1 opt;
			case $opt in
				1) echo "COMMANDS Stage 3 - Part 1" && wait;;
				2) echo "COMMANDS Stage 3 - Part 2" && wait;;
				3) echo "COMMANDS Stage 3 - Part 2" && wait;;
				/q | q | 0)break;;
				*) ;;
			esac
		done
