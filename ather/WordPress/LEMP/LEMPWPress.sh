...
phpV="7.0"
function TIMER() {
  if [[ "$1" =~ ^[[:digit:]]+$ ]]; then T="$1"; else if [ -z "$1" ]; then T="6"; else T="10"; fi; fi;
...
sed -i -e "s/example.com/$DOMAIN/" /etc/nginx/sites-available/$DOMAIN
sed -i -e "s/www.example.com/www.$DOMAIN/" /etc/nginx/sites-available/$DOMAIN
...
ROBOTS_TXT_DOM="/var/www/$DOMAIN/robots.txt"
touch /var/www/$DOMAIN/robots.txt
...
mv /Temp/wordpress/* /var/www/"$DOMAIN"/
