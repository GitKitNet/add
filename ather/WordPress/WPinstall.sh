...
TIMER() {
  ...
  T="9";
  secs="$((1 * ${T}))"; 
  while [ $secs -gt 0 ]; do echo -ne "\t${INF}\t $secs\033[0K\r";sleep 1;: $((secs--)); done;
}
...
## -------------------------------------
DATA() { DATA=$(date +%Y-%m-%d_%k%M%S); echo "$DATA"; }
...
echo -en "
List of required packeges:
  nano, zip, unzip, mc, htop, fail2ban, apache2, 
  php, mysql, php curl, phpmyadmin, wget, curl
"
