...
function ARHOS() {
  OSI="$(cat /etc/*release |grep '^ID=' |sed 's/"//g' |awk -F= '{print $2 }' )";
  REL="$(cat /etc/*release |grep '^VERSION_ID=' |sed  's/"//g' |awk -F= '{print $2 }' )";
  if [[ "$1" == "OSI" ]]; then echo "${OSI}"; fi;
  if [[ "$1" == "REL" ]]; then echo "${REL}"; fi;
  if [[ -z "$1" ]]; then echo "${REL} ${OSI}"; fi;
};
ARHOS OSI
ARHOS REL
...
function title() { clear; echo "${title}"; pause; }
function myip() {
  ...
  if [ "$ipE" == "$ipW" ]; then myip="$ipE"; else if [ "$ipH" == "$ipW" ]; then myip="$ipH"; else myip="$ipW"; fi; fi;
  echo ${myip};
}

function TIMER() {
  if [[ -z "$1" ]]; then T=10;else T=$1; fi
  C='\033[0K\r' && secs="$((1 * ${T}))" && while [ $secs -gt 0 ]; do echo -ne "${YELLOW} Wait..: ${RED}$secs${C}${NC}" && sleep 1 && : $((secs--)); done;
}
...
CURL=$(dpkg-query -W -f='${Status}' curl 2>/dev/null | grep -c "ok installed")
  ...
  ;;
    *) echo -e "${RED} Packeges check is ignored!Please be aware, that apache2, mysql, phpmyadmin and other software may not be installed!${NC}"
  ;;
esac
