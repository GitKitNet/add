...
function TIMER() {
  ...
  secs="$((1 * ${T}))";
  while [ $secs -gt 0 ]; do
    echo -ne " ${YELLOW}${W}: ${RED}$secs${C}${NC}";
    sleep 1 && : $((secs--));
  done;
}
...
UNZIP="$(dpkg-query -W -f='${Status}' unzip 2>/dev/null | grep -c 'ok installed')"
  if [ "$UNZIP" -eq 0 ]; then
    echo -e "${YELLOW}Installing unzip${NC}" && apt-get install unzip --yes;
...
function DOWNLOADING_WPplugins() {
TIMER
